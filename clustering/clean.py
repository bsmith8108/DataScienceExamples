...
def create_rows_list_of_lists_with_cols(list_of_dicts, cols_to_include):
    headers = gd.get_headers()
    final = []
    for x in range(len(list_of_dicts)):
	final.append([])
...
filename = "oklahoma_earthquakes.cleaned.csv"
list_of_dicts = gd.get_data_list_of_dicts()
new_headers = get_headers_with_cols([0,2,3,4,11,29])
rows_list_of_lists = create_rows_list_of_lists_with_cols(list_of_dicts, [0,2,3,4,11,29])
